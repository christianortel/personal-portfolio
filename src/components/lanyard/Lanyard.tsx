...
const cardGLB = "/card.glb?v=2026-07-11-2";
...
function Band({
  ...
  const cardMap = useMemo(() => {
    const baseMap = (materials.base?.map ?? null) as THREE.Texture | null;
    ...
    if (!ctx) return baseMap ?? new THREE.Texture();
    if (baseImg) {
  ...
  useRopeJoint(fixed, j1, [[0, 0, 0], [0, 0, 0], 0.55] as any);
  useRopeJoint(j1, j2, [[0, 0, 0], [0, 0, 0], 0.55] as any);
  useRopeJoint(j2, j3, [[0, 0, 0], [0, 0, 0], 0.55] as any);
